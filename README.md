...
当前是可运行的本地首版：中文变量、四则运算、百分比、单位换算、分段汇总、笔记管理、自动保存和文本导入导出。设置中可选择存储位置与计算器边栏或弹窗显示；侧栏底部以太阳/月亮图标在纸白与午夜两套配色间切换；关于信息包含版本、构建时间和 GitHub 链接。
...
构建产物在 `target/release/bundle/`（Cargo target 目录通过 `.cargo/config.toml` 统一到仓库根）。当前以 macOS 为首个验证平台；Windows/Linux 打包、签名、公证、自动更新尚未接入。
